...
impl AuditEvent {
    ...
    /// `notice`-severity event (a deliberate, notable change).
...
mod tests {
    ...
    fn disabled_sink_is_noop_and_never_drops() {
        ...
        for _ in 0..1000 {
            sink.emit(AuditEvent::notice("dns.zone.edit", "a@b", "w33d.xyz", "ok"));
        }
    ...
    async fn emit_never_blocks_when_sink_unreachable() {
        ...
        for _ in 0..(QUEUE_CAPACITY * 8) {
            sink.emit(AuditEvent::notice("dns.zone.edit", "u", "w33d.xyz", "add A foo"));
        }
        assert!(
            sink.dropped() > 0,
