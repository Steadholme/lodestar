//! Lodestar entry point: init state from env (select store, seed, load resolver), start the DNS
//! listeners + periodic reload, then serve the admin dashboard.
//!
//! Also exposes a dependency-free `lodestar healthcheck` subcommand used as the container
...
async fn main() {
    ...
    let state = match lodestar::build_state_from_env().await {
        Ok(state) => state,
        Err(e) => {
    ...
    // Bind the DNS listeners (ALT port :5353) and start the periodic zone reload.
    if let Err(e) = lodestar::start_background(&state).await {
        tracing::error!(error = %e, "failed to start DNS background services");
        std::process::exit(1);
    }
    ...
    let app = lodestar::app(state);

    let listener = tokio::net::TcpListener::bind(addr)
        .await
        .unwrap_or_else(|e| panic!("failed to bind {addr}: {e}"));

    tracing::info!(%addr, "Lodestar admin dashboard listening (authoritative DNS + zone admin)");
    axum::serve(listener, app).await.expect("server error");
}

/// GET `/healthz` over a raw TCP socket. Returns process exit code (0 = healthy).
fn run_healthcheck() -> i32 {
    let bind_addr = std::env::var("BIND_ADDR").unwrap_or_else(|_| "127.0.0.1:9110".to_string());
    // Healthcheck always talks to the loopback regardless of the bind interface.
    let port = bind_addr.rsplit(':').next().unwrap_or("9110");
