...
//!
//! Lodestar does NO login of its own. The admin dashboard sits behind a Sluice `auth=sso` route,
//! where the gateway runs the OIDC browser login against Keystone, STRIPS any inbound `X-Auth-*`,
//! and injects the verified `X-Auth-Subject` / `X-Auth-Email` / `X-Auth-Scope`. Because Lodestar is
//! internal-only (never publicly reachable), it TRUSTS those headers as the authenticated operator.
//! (The DNS protocol on `:5353` is a separate surface entirely — it is unauthenticated by design,
//! as authoritative DNS is, and never reads these headers.)
//!
//! State-changing POSTs (add / delete record) are double-submit CSRF protected: a random token
//! lives in a JS-readable `__Host-csrf` cookie AND in a hidden form field; the POST is accepted only
...
/// The gateway-asserted scope string, if any (carried only for audit detail).
pub fn scope(headers: &HeaderMap) -> Option<String> {
    header_value(headers, HEADER_SCOPE)
}

/// The actor label for audit events: the email if present, else the subject, else `anonymous`.
pub fn actor(headers: &HeaderMap) -> String {
    email(headers)
        .or_else(|| subject(headers))
