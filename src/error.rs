...
#[derive(Debug, Error)]
pub enum AppError {
    /// Malformed/incomplete form input (empty name, bad record value, etc.).
    ...
    /// Zone-name collision on create (the UNIQUE(name) guard).
    #[error("conflict: {0}")]
    Conflict(String),
...
/// Store failures collapse to their HTTP shape: a zone conflict is a 409, everything else 500.
